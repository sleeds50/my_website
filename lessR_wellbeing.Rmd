...
hours using the `lessr` package [@R-lessR] which I have recently found to be
very useful.
...
The wellbeing data are included in the `UsingR` package [@R-UsingR] and
originally used [here](http://prcweb.co.uk/lab/what-makes-us-happy/), where you
can see interactive correlations with wellbeing and variables of your choice.
There are also links to the data origin at [The National Accounts of
...
`Sort()` function in `lessR` (all code for this document can be found in the
[Appendix](#Appendix)). For the purposes of using `lessR`, the
...
3. The adjusted R-squared value is $(R^2_adj = `r round(r$Rsqadj, 4)`)$.
...
# Appendix: All code for this article {#Appendix}
