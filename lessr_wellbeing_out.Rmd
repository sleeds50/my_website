...
```{r}
regPlot(r, 1, pred.intervals=FALSE, NULL)  # 1: scatter plot 
```
...
```{r}
regPlot(r, 1, NULL)  # 1: scatter plot with prediction intervals
```
...
```{r}
regPlot(r, 2, NULL)  # 2: distribution of residuals
```
...
```{r}
regPlot(r, 3, NULL)  # 3: scatter plot of fitted with residuals
```
