...
variation and function in discourse using interpretive repertoire, subject
positions and ideological dilemma to help understand how volunteer identities
are constructed. In the **method** section, the criteria and process for
sourcing the data via the *Nexis* database is explained. The resulting corpus
(10 items; 5590 words) included articles in local newspapers from England (4),
Scotland (3), Wales (1); and National papers (2). No suitable items were
available for Northern Ireland due to content or word count constraints.
...
**Analyisis** (see Figure 1 for a visual summary): It became clear that the news
...
```{r plot, fig.cap="__Figure 1.__ Interpretive Repertoire Instances for each Perspective by Newspaper Region"}

ggplot(interep, aes(Description,fill=Pos_Neg))+
    geom_dotplot(dotsize = 2.5, 
    ...
    labs(x = "Interpretive Rerpertoire",
         y = "",
...
perspective (horizontal). For example, negative identities are given to
volunteers by management in three regional areas with
"`r interep$Description[20]`"; volunteers were the most positive *and* negative
about themselves; and the Welsh regional paper could be seen as the most
positive with no negative subject positioning.
...
Use the [email link](mailto:sleeds40@gmail.com) above for questions or further information.
