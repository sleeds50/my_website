...
# author: "Stuart Leeds"
date: "2021-07-14, _updated 2024-10-24_"
output: 
  html_document:
    toc: FALSE
---
...
There's little point presenting a plot with three points on it, so Coordinate
plot 5 below shows the locations in Surrey/Middlesex and Somerset, with the
expanse between.
...
meanCoords |>
  ...
  kable_classic_2(full_width = FALSE, position = "center") |>
    footnote("Update shows new locations. 
             Originally: Worldwide place: Génicourt-sur-Meuse, France; England place: Cheshunt, Hertfordshire")
```
...
ggplot(meanCoords, aes(x = Longitude, y = Latitude)) +
  ...
  labs(
    ...
    caption = "Note. Coordinates intercept at Greenwich, London. Before update: 
    England mean = Cheshunt, Hertfordshire; World mean =  Génicourt-sur-Meuse, France"
  ) +
  theme(plot.caption = element_text(hjust = 0, face = "italic"))
```


I must admit, I quite fancy **Génicourt-sur-Meuse and Neuvilly en Argonne,
France**. I've never been to either, but it sounds like it would suit me.
Ironically, I worked in Cheshunt (the original location before the update!) for
...
medianCoords |>
  ...
  kable_classic_2(full_width = FALSE, position = "center") |> 
    footnote("Update shows new locations. 
    Originally: World place: Upminster, Essex. England place: Stapleford Abbotts, Essex")
```
...
ggplot(medianCoords, aes(x = Longitude, y = Latitude)) +
  ...
  labs(
    ...
    caption = "Note. Coordinates intercept at Greenwich, London
            * = Rush Green, Dagenham. Before update: 
            England median = Stapleford Abbotts; World median = Upminster"
  ) +
  theme(plot.caption = element_text(hjust = 0, face = "italic"))
```
...
> The updated ones are not that far away either!
...
__Rush Green__ coordinates. Then, I found out that `distm` needs the longitude
...
So the calculation returns like this for Lambourne End: 
...
> __Lambourne End = 8.60Km (5.34 miles) away.__
> <small>Originally: Stapleford Abbotts = 8.73Km (5.43 miles)</small>

to the median of worldwide locations:  

> __Dartford = 12.89Km (8.01 miles) away.__
> <small>Originally: Upminster = 5.58Km (3.47 miles)</small>
